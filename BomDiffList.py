...
def get_data():
    ...
    for item in range(1, nrows-1):
        data_now_pn.append(sheet_now.cell(item, 1).value)
    ...
    for i in data_display.keys():
        ...
        count += 1
    WorkBook.close()
    tkMessageBox.showinfo('提示'.decode('gbk'), '处理BOM和兼容性列表的不同处的结果已经生成，请去%s路径查看.如果无需其他动作，请点击退出按钮退出程序'.decode('gbk') % filename_output)
...
Tkinter.Entry(frame_middle_bottom, textvariable=var_char_entry_filename_now_need_filter, width=40).pack(side=Tkinter.LEFT)
Tkinter.Button(frame_middle_bottom, text='选择文件'.decode('gbk'), command=get_filename_now, width=20).pack(side=Tkinter.RIGHT)
...
Tkinter.Button(frame_middle_bottom_2, text='选择文件'.decode('gbk'), command=set_filename_output, width=20).pack(side=Tkinter.RIGHT)
...
box_set_sheet = ttk.Combobox(frame_middle_3_bottom, textvariable=var_char_combox_sheet_now, values=['0', '1', '2','3','4','5','6'], width=30)
